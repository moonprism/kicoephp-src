...
class File extends SplFileInfo{
    ...
    public function validate()
    {
        ...
        // 验证文件类型
        if (
            isset($this->file_veri['type']) &&
            (
                is_string($this->file_veri['type'])?
                    ($this->up_info['type'] != $this->file_veri['type'] ):
                    ( !in_array($this->up_info['type'], $this->file_veri['type']) ) ) ) {
            throw new Exception("file type ：", '...');
        }
        // 验证文件后缀
